...
class combined(Agent):
    def __init__(self, args, env_args, load_model, actor_path, critic_path):
        super(combined, self).__init__(args, env_args=env_args)
        self.args = args
        ...
        # Logging
        self.model_logs = torch.zeros(8, device=self.device)
        self.LogExperiment = LogExperiment(args)

    def train_pi_PPO(self):
        print('Running PPO Policy Update...')
        ...
        return mean_pi_grad, temp_loss_log, 0

    def train_pcpo(self):
        print('Running PCPO updated Policy Update...')
        ...
        while start_idx<buffer_len:
            ...
            # finding the step direction
            Fvp = Fvp_fim
            stepdir = conjugate_gradients(Fvp, -loss_grad, 10) # H^-1.g

            # finding cost loss
            c_theta = ratios * cost_advantages_batch
            """o need for the- sign in cost loss, as we are minimizing the cost function"""
    ...
    def train_pi_cpo(self):
        ...
        for i in range(self.train_pi_iters):
            ...
            # finding the step direction
            Fvp = Fvp_fim
            stepdir = conjugate_gradients(Fvp, -loss_grad, 20) # H^-1.g
            ...
            cost_grads = torch.autograd.grad(cost_loss, self.policy.Actor.parameters())
            cost_loss_grad = torch.cat([grad.view(-1) for grad in cost_grads]) 
            cost_stepdir = conjugate_gradients(Fvp, -cost_loss_grad, 20) # H^-1.a
            ...
            """Following two different implementations work in the same way"""
    ...
    def update(self):
        ...
        if(self.completed_interactions < 100000):
            print(self.completed_interactions)
            self.model_logs[0], self.model_logs[5], self.model_logs[7] = self.train_pi_PPO()
        else:
            self.model_logs[0], self.model_logs[5], self.model_logs[7] = self.train_pi_cpo()
